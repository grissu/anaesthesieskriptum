...
{index}` <Lambert-Eaton Syndrom>`
{index}` <Syndrom; Lambert-Eaton>`
...
Therapie
:   -   Symptomatisch:
        Mestinon 3 x 60 mg p.o. (= Pyridostigmin-Bromid) = Acetylcholin-Esterase-Inhibitoren oder Ubretid (Distigminbromid)
...
:   Mestinon am OP-Tag pausieren oder präop um 20% reduzieren!
    (Besser leicht myasthen als cholinergisch);
...
Myasthene Krise
:   {index}` <Myasthene Krise>`
    {index}` <Krise; myasthene>`
...
Cholinerge Krise
:   {index}` <Cholinerge Krise>`
    {index}` <Krise; cholinerge >`
...
{index}` <Gullain-Barre-Syndrom>`
{index}` <Syndrom; Gullain-Barre->`
