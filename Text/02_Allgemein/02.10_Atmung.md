...
{index}` <Ventilations-Perfusions-Verhältnis>`
{index}` <Verhältnis; Ventilations-Perfusions->`
...
:   {index}` <Rechtsverschiebung>`
    {index}` <Bohr-Effekt>`
    {index}` <Effekt; Bohr->`
    bei Azidose, Fieber, Hyperkapnie mit leichterer O₂ Abgabe ans Gewebe.

Hüfnersche Zahl
:   {index}` <Hüfnersche Zahl>`
    {index}` <Zahl; Hüfnersche>`
...
:   {index}` <Respiratorischer Quotient>`
    {index}` <Quotient; Respiratorischer>`
    {index}` <RQ>`
    CO₂-Produktion: Sauerstoffaufnahme = 0,8 normal

Oxygenierungsindex (Horowitz-Index)
:   {index}` <Oxygenierungsindex>`
    {index}` <Index; Oxygenierungs->`
    {index}` <Horowitz-Index>`
    {index}` <Index; Horowitz->`
    paO2/FiO₂ = normal > 450
...
:   {index}` <Hagen-Poiseulle>`
    ...
    *Strömungswiderstand* ist proportional zur Länge und umgekehrt proportional zur 4 Potenz des Radius (Der Widerstand steigt mit der Länge und mit der *vierten Potenz* der Gefäßverengung).
    ...
    -   Gefäßzugänge:
        Praktisch ist ein kurzer Venflon besser als ein Schenkel des ZVK mit gleichem Durchmesser.
    -   Atemweg:
